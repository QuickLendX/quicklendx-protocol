...
export class FormValidator {
  ...
  static validatePartial(schema: z.ZodObject<any>, _data: unknown): unknown {
    try {
      return schema.partial().parse(_data);
    } catch (error) {
      if (error instanceof z.ZodError) {
        const messages = error.errors.map((err) => err.message).join(", ");
        throw new ValidationError(messages, {
  ...
  const validate = (
    ...
    try {
      ...
      if (error instanceof ValidationError) {
        const fieldErrors =
          (error.context?.additionalData?.fieldErrors as Record<
            string,
            string
          >) || {};
