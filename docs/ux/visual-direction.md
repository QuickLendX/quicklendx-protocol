...
- **Risk Color Semantics**: Red is reserved for _loss_ or _blockers_. Amber is for _precautions_. Never use brand colors for risk-sensitive alerts to avoid confusion.
- **No Misleading Guarantees**: Use terms like "Expected Yield" instead of "Guaranteed Return." Avoid using overly celebratory animations for risky investment actions. For detailed copy rules, see the Content Style Guide.
...
1. **Assumed**: Users understand that once a bid is accepted, the invoice is legally/on-chain committed.
   - **Validation**: The "Accept Bid" modal must display a summary of the commitment: _"By accepting, you lock [Amount] XLM until [Date]."_
