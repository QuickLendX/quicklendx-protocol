...
pub struct TokenConfig { pub token: TokenAddress, pub enabled: bool, pub version: u32 }
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
pub fn apply_token_update(admin: TokenAddress, expected_admin: TokenPolicyAdmin, current: Option<TokenConfig>, token: TokenAddress, enabled: bool, expected_version: u32) -> Result<(TokenConfig, TokenConfigEvent), TokenPolicyError> {
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct TokenUpdateRequest { pub token: TokenAddress, pub enabled: bool, pub expected_version: u32 }
...
pub fn apply_batch_updates(admin: TokenAddress, expected_admin: TokenPolicyAdmin, list: &mut TokenAllowlist, updates: &[TokenUpdateRequest]) -> Result<TokenPolicySnapshot, TokenPolicyError> {
    ...
    while index < updates.len() {
        ...
        let (next, _) = apply_token_update(admin, expected_admin, current, update.token, update.enabled, update.expected_version)?;
...
impl TokenAllowlist {
    ...
    pub fn remove(&mut self, token: TokenAddress, expected_version: u32) -> Result<TokenConfig, TokenPolicyError> {
        ...
        let next = TokenConfig { token, enabled: false, version: current.version.checked_add(1).ok_or(TokenPolicyError::InvalidVersion)? };
        self.replace(next)?;
        Ok(next)
    }
...
mod tests {
    ...
    #[test] fn enabled_is_supported() { assert!(authorize_token(Some(TokenConfig { token: A, enabled: true, version: 1 })).is_ok()); }
    #[test] fn disabled_is_removed() { assert_eq!(authorize_token(Some(TokenConfig { token: A, enabled: false, version: 1 })), Err(TokenPolicyError::Removed)); }
    #[test] fn funded_removed_is_readable() { assert!(authorize_payment(Some(TokenConfig { token: A, enabled: false, version: 2 }), true).is_ok()); }
    #[test] fn new_removed_is_rejected() { assert_eq!(authorize_payment(Some(TokenConfig { token: A, enabled: false, version: 2 }), false), Err(TokenPolicyError::Removed)); }
    #[test] fn add_starts_at_one() { assert_eq!(apply_token_update(ADMIN.address, ADMIN, None, A, true, 0).unwrap().0.version, 1); }
    #[test] fn remove_increments_version() { let (next, _) = apply_token_update(ADMIN.address, ADMIN, Some(TokenConfig { token: A, enabled: true, version: 1 }), A, false, 1).unwrap(); assert_eq!(next.version, 2); }
    #[test] fn readd_increments_version() { let (next, _) = apply_token_update(ADMIN.address, ADMIN, Some(TokenConfig { token: A, enabled: false, version: 2 }), A, true, 2).unwrap(); assert_eq!(next.version, 3); }
    #[test] fn wrong_admin_is_rejected() { assert_eq!(apply_token_update(B, ADMIN, None, A, true, 0), Err(TokenPolicyError::Unauthorized)); }
    #[test] fn zero_admin_is_rejected() { assert_eq!(apply_token_update([0; 32], ADMIN, None, A, true, 0), Err(TokenPolicyError::InvalidAdmin)); }
    #[test] fn stale_version_is_rejected() { assert_eq!(apply_token_update(ADMIN.address, ADMIN, Some(TokenConfig { token: A, enabled: true, version: 4 }), A, false, 3), Err(TokenPolicyError::VersionConflict)); }
    #[test] fn wrong_token_is_rejected() { assert_eq!(apply_token_update(ADMIN.address, ADMIN, Some(TokenConfig { token: A, enabled: true, version: 1 }), B, false, 1), Err(TokenPolicyError::VersionConflict)); }
    #[test] fn old_and_new_values_are_emitted() { let (_, e) = apply_token_update(ADMIN.address, ADMIN, Some(TokenConfig { token: A, enabled: true, version: 1 }), A, false, 1).unwrap(); assert_eq!((e.old_enabled, e.new_enabled), (true, false)); }
    #[test] fn replay_helper_accepts_exact_record() { let c = TokenConfig { token: A, enabled: true, version: 1 }; assert!(replay_update_is_noop(Some(c), c, 1)); }
    #[test] fn replay_helper_rejects_version_change() { let c = TokenConfig { token: A, enabled: true, version: 1 }; assert!(!replay_update_is_noop(Some(c), c, 2)); }
    ...
    #[test] fn allowlist_inserts_new_token() { let mut list = TokenAllowlist::empty(); assert!(list.insert(TokenConfig { token: A, enabled: true, version: 1 }).is_ok()); assert_eq!(list.length, 1); }
    #[test] fn allowlist_rejects_duplicate_insert() { let mut list = TokenAllowlist::empty(); list.insert(TokenConfig { token: A, enabled: true, version: 1 }).unwrap(); assert_eq!(list.insert(TokenConfig { token: A, enabled: false, version: 1 }), Err(TokenPolicyError::VersionConflict)); }
    #[test] fn allowlist_replaces_next_version() { let mut list = TokenAllowlist::empty(); list.insert(TokenConfig { token: A, enabled: true, version: 1 }).unwrap(); assert!(list.replace(TokenConfig { token: A, enabled: false, version: 2 }).is_ok()); assert_eq!(list.find(A).unwrap().enabled, false); }
    #[test] fn allowlist_rejects_skipped_version() { let mut list = TokenAllowlist::empty(); list.insert(TokenConfig { token: A, enabled: true, version: 1 }).unwrap(); assert_eq!(list.replace(TokenConfig { token: A, enabled: false, version: 3 }), Err(TokenPolicyError::VersionConflict)); }
    #[test] fn allowlist_replace_unknown_is_unsupported() { let mut list = TokenAllowlist::empty(); assert_eq!(list.replace(TokenConfig { token: A, enabled: true, version: 1 }), Err(TokenPolicyError::Unsupported)); }
    ...
    #[test] fn batch_adds_multiple_tokens() { let mut list = TokenAllowlist::empty(); let updates = [TokenUpdateRequest { token: A, enabled: true, expected_version: 0 }, TokenUpdateRequest { token: B, enabled: true, expected_version: 0 }]; let result = apply_batch_updates(ADMIN.address, ADMIN, &mut list, &updates).unwrap(); assert_eq!((result.configured, result.enabled), (2, 2)); }
    #[test] fn batch_rejects_zero_token() { let mut list = TokenAllowlist::empty(); let updates = [TokenUpdateRequest { token: [0; 32], enabled: true, expected_version: 0 }]; assert_eq!(validate_update_batch(ADMIN.address, ADMIN, &updates), Err(TokenPolicyError::Unsupported)); assert_eq!(list.length, 0); }
    #[test] fn batch_rejects_wrong_admin() { let mut list = TokenAllowlist::empty(); let updates = [TokenUpdateRequest { token: A, enabled: true, expected_version: 0 }]; assert_eq!(apply_batch_updates(B, ADMIN, &mut list, &updates), Err(TokenPolicyError::Unauthorized)); assert_eq!(list.length, 0); }
    ...
    #[test] fn contains_enabled_distinguishes_disabled() { let mut list = TokenAllowlist::empty(); list.insert(TokenConfig { token: A, enabled: false, version: 1 }).unwrap(); assert!(!list.contains_enabled(A)); }
    #[test] fn contains_enabled_returns_false_for_unknown() { assert!(!TokenAllowlist::empty().contains_enabled(A)); }
    #[test] fn remove_increments_version() { let mut list = TokenAllowlist::empty(); list.insert(TokenConfig { token: A, enabled: true, version: 1 }).unwrap(); let removed = list.remove(A, 1).unwrap(); assert_eq!(removed.version, 2); assert!(!list.contains_enabled(A)); }
    #[test] fn remove_requires_current_version() { let mut list = TokenAllowlist::empty(); list.insert(TokenConfig { token: A, enabled: true, version: 1 }).unwrap(); assert_eq!(list.remove(A, 0), Err(TokenPolicyError::VersionConflict)); }
    ...
    #[test] fn enabled_count_uses_snapshot() { let mut list = TokenAllowlist::empty(); list.insert(TokenConfig { token: A, enabled: true, version: 1 }).unwrap(); list.insert(TokenConfig { token: B, enabled: false, version: 1 }).unwrap(); assert_eq!(list.enabled_count(), 1); }
    #[test] fn empty_allowlist_validates() { assert!(TokenAllowlist::empty().validate().is_ok()); }
    #[test] fn distinct_allowlist_entries_validate() { let mut list = TokenAllowlist::empty(); list.insert(TokenConfig { token: A, enabled: true, version: 1 }).unwrap(); list.insert(TokenConfig { token: B, enabled: true, version: 1 }).unwrap(); assert!(list.validate().is_ok()); }
}
