...
mod tests {
    ...
    fn record(status: VerificationStatus, expiry: u64, version: u32) -> Option<KycRecord> {
        Some(KycRecord {
            status,
            expires_at: expiry,
            version,
    ...
    fn every_status_has_a_stable_error() {
        assert_eq!(
            require_eligible(None, 50, 0),
            Err(KycEligibilityError::Missing)
        );
        assert_eq!(
            require_eligible(record(VerificationStatus::Pending, 100, 1), 50, 0),
            Err(KycEligibilityError::Pending)
        );
        assert_eq!(
            require_eligible(record(VerificationStatus::Rejected, 100, 1), 50, 0),
            Err(KycEligibilityError::Revoked)
        );
        assert_eq!(
            require_eligible(record(VerificationStatus::Verified, 49, 1), 50, 0),
            Err(KycEligibilityError::Expired)
        );
        assert_eq!(
            require_eligible(record(VerificationStatus::Verified, 0, 1), 0, 0),
    ...
    fn verified_boundaries_are_identical_for_all_actions() {
        ...
        for action in actions {
            let actor = if action == KycDependentAction::CreateInvoice {
                KycActor::Business
            } else {
                KycActor::Investor
            };
            assert!(authorize_action(
                record(VerificationStatus::Verified, 100, 7),
                actor,
                action,
                99,
                0
        ...
        for action in actions {
            assert_eq!(
                authorize_before_side_effect(
                    record(VerificationStatus::Pending, 100, 1),
                    KycActor::Investor,
                    action,
                    1,
                    false,
    ...
    fn revoked_cannot_cross_any_nonterminal_entrypoint() {
        ...
        for action in actions {
            assert_eq!(
                authorize_before_side_effect(
                    record(VerificationStatus::Rejected, 100, 1),
                    KycActor::Investor,
                    action,
                    1,
                    false,
    ...
    fn version_replay_is_rejected() {
        for next in [0, 1, 2, 9] {
            assert_eq!(can_update_verification(false, 2, next), next > 2);
        }
    }
