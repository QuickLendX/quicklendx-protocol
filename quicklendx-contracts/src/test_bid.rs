...
/// Core Test: Accept bid requires the bid to belong to the provided invoice
#[test]
fn test_accept_bid_rejects_mismatched_invoice_pair() {
    let (env, client) = setup();
    env.mock_all_auths();
    let admin = Address::generate(&env);
    let _ = client.set_admin(&admin);
    let business = Address::generate(&env);
    let (currency, investor) = setup_token_and_investor(&env, &client, 100_000);

    let invoice_id_1 =
        create_verified_invoice_with_token(&env, &client, &business, 10_000, &currency);
    let invoice_id_2 =
        create_verified_invoice_with_token(&env, &client, &business, 10_000, &currency);
    let bid_id = client.place_bid(&investor, &invoice_id_2, &5_000, &6_000);

    let result = client.try_accept_bid(&invoice_id_1, &bid_id);
    assert!(
        result.is_err(),
        "accept_bid must reject mismatched invoice/bid pairs"
    );

    let err = result.unwrap_err().unwrap();
    assert_eq!(err, QuickLendXError::Unauthorized);
    assert_eq!(
        client.get_invoice(&invoice_id_1).status,
        crate::invoice::InvoiceStatus::Verified
    );
    assert_eq!(
        client.get_invoice(&invoice_id_2).status,
        crate::invoice::InvoiceStatus::Verified
    );
...
fn test_cleanup_expired_bids_idempotent() {
    ...
    assert_eq!(
        removed_first, 2,
        "First cleanup should remove 2 expired bids"
    );
...
fn test_get_ranked_bids_excludes_expired() {
    ...
    assert_eq!(
        ranked_before.len(),
        3,
        "Should have 3 ranked bids initially"
    );
    ...
    assert_eq!(
        ranked_after.len(),
        0,
        "Ranked bids should be empty after expiration"
    );
}
...
fn test_get_best_bid_excludes_expired() {
    ...
    assert_eq!(
        best_before.unwrap().investor,
        investor2,
        "Best bid should be investor2"
    );
    ...
    assert!(
        best_after.is_none(),
        "Best bid should be None after all bids expire"
    );
}
...
fn test_place_bid_cleans_up_expired_before_placing() {
    ...
    assert_eq!(
        placed_after.len(),
        1,
        "Should have only 1 placed bid (new one)"
    );
...
fn test_partial_expiration_cleanup() {
    ...
    assert_eq!(
        bid_2_status.status,
        BidStatus::Placed,
        "Second bid should still be placed"
    );
    ...
    assert_eq!(
        placed_bids.len(),
        2,
        "Should have 2 placed bids (second and third)"
    );
}
...
fn test_cleanup_triggered_on_query_after_expiration() {
    ...
    env.ledger()
        .set_timestamp(env.ledger().timestamp() + (1 * 24 * 60 * 60));
    ...
    assert_eq!(
        placed_bids.len(),
        1,
        "Should have only 1 placed bid after cleanup"
    );
...
fn test_bid_at_exact_expiration_not_expired() {
    ...
    assert_eq!(
        placed_bids.len(),
        1,
        "Bid at exact expiration should still be placed"
    );
    ...
    assert_eq!(
        bid_status.status,
        BidStatus::Placed,
        "Bid should still be placed at exact expiration"
    );
}
...
fn test_bid_one_second_past_expiration_expired() {
    ...
    assert_eq!(
        bid_status.status,
        BidStatus::Expired,
        "Bid should be expired one second past expiration"
    );
}
...
fn test_withdrawn_bids_not_affected_by_expiration() {
    ...
    assert_eq!(
        bid_1_status.status,
        BidStatus::Withdrawn,
        "Withdrawn bid should remain withdrawn"
    );
...
fn test_cancelled_bids_not_affected_by_expiration() {
    ...
    assert_eq!(
        bid_1_status.status,
        BidStatus::Cancelled,
        "Cancelled bid should remain cancelled"
    );
...
fn test_expiration_cleanup_isolated_per_invoice() {
    ...
    assert_eq!(
        bid_1_status.status,
        BidStatus::Expired,
        "Invoice 1 bid should be expired"
    );
    ...
    assert_eq!(
        bid_2_status.status,
        BidStatus::Placed,
        "Invoice 2 bid should still be placed"
    );
    ...
    assert_eq!(
        bid_2_status_after.status,
        BidStatus::Expired,
        "Invoice 2 bid should now be expired"
    );
}
...
fn test_expired_bids_removed_from_invoice_list() {
    ...
    assert_eq!(
        bids_after.len(),
        0,
        "Expired bids should be removed from invoice list"
    );
}
...
fn test_ranking_after_all_bids_expire() {
    ...
    assert_eq!(
        ranked_before.get(0).unwrap().investor,
        investor2,
        "Best bid should be investor2"
    );
    ...
    assert_eq!(
        ranked_after.len(),
        0,
        "Ranking should be empty after all bids expire"
    );
...
fn test_multiple_investors_place_bids_on_same_invoice() {
    ...
    assert!(
        all_bid_ids.iter().any(|bid| bid.bid_id == bid_id1),
        "bid_id1 should be in list"
    );
    assert!(
        all_bid_ids.iter().any(|bid| bid.bid_id == bid_id2),
        "bid_id2 should be in list"
    );
    assert!(
        all_bid_ids.iter().any(|bid| bid.bid_id == bid_id3),
        "bid_id3 should be in list"
    );
    assert!(
        all_bid_ids.iter().any(|bid| bid.bid_id == bid_id4),
        "bid_id4 should be in list"
    );
    assert!(
        all_bid_ids.iter().any(|bid| bid.bid_id == bid_id5),
...
fn test_business_accepts_one_bid_others_remain_placed() {
    ...
    let invoice_id =
        create_verified_invoice_with_token(&env, &client, &business, 100_000, &currency);
