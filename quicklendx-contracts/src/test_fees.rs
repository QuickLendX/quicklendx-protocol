...
fn test_calculate_transaction_fees_base_case() {
    ...
    fn test_distribute_revenue_below_minimum() {
        ...
        client
            .configure_revenue_distribution(&admin, &treasury, &5000, &3000, &2000, &false, &10000);
    ...
    fn test_update_fee_structure_base_fee_bps_variations() {
        ...
        let fee_zero =
            client.update_fee_structure(&admin, &FeeType::Platform, &0, &10, &1000, &true);
        assert_eq!(fee_zero.base_fee_bps, 0);

        // Test mid-range base_fee_bps
        let fee_mid =
            client.update_fee_structure(&admin, &FeeType::Platform, &500, &10, &1000, &true);
        assert_eq!(fee_mid.base_fee_bps, 500);

        // Test maximum valid base_fee_bps (1000 = 10%)
        let fee_max =
            client.update_fee_structure(&admin, &FeeType::Platform, &1000, &10, &1000, &true);
    ...
    fn test_update_fee_structure_min_fee_variations() {
        ...
        // Test min_fee = 0
        let fee_zero =
            client.update_fee_structure(&admin, &FeeType::Platform, &200, &0, &1000, &true);
        assert_eq!(fee_zero.min_fee, 0);

        // Test min_fee = 1
        let fee_one =
            client.update_fee_structure(&admin, &FeeType::Platform, &200, &1, &1000, &true);
    ...
    fn test_update_fee_structure_sets_updated_by() {
        ...
        assert!(
            result.is_err(),
            "Zero amount must return InvalidAmount error"
        );
    }
    let admin = setup_admin_init(&env, &client);
