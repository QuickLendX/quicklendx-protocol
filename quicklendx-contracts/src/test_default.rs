...
fn test_default_after_grace_period() {
    ...
    let invoice_id = create_and_fund_invoice(&env, &client, &admin, &business, &investor, amount, due_date);
...
fn test_default_investment_status_update() {
    ...
    assert_eq!(defaulted_investment.status, crate::investment::InvestmentStatus::Defaulted);
}
...
fn test_multiple_invoices_default_handling() {
    ...
    let invoice1_id = create_and_fund_invoice(&env, &client, &admin, &business, &investor, amount, due_date);
    let invoice2_id = create_and_fund_invoice(&env, &client, &admin, &business, &investor, amount, due_date + 86400);
    ...
    assert_eq!(client.get_invoice(&invoice1_id).status, InvoiceStatus::Defaulted);
    
    // Second invoice should not default yet
    let result = client.try_mark_invoice_defaulted(&invoice2_id, &Some(grace_period));
    assert!(result.is_err());
    assert_eq!(client.get_invoice(&invoice2_id).status, InvoiceStatus::Funded);
}
