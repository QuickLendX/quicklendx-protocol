...
fn test_pause_reads_available() {
    let env = Env::default();
    let (client, admin, funded_invoice_id, active_invoice_id, investor) =
        setup_and_fund_invoice(&env);
    ...
    assert_eq!(
        accept_bid_err.unwrap_err().unwrap(),
        QuickLendXError::ContractPaused
    );
    ...
    assert_eq!(
        inv_after.amount, inv_before.amount,
        "get_invoice should return same state"
    );
    assert_eq!(inv_after.status, inv_before.status);

    let best_bid_after = client.get_best_bid(&active_invoice_id);
    assert_eq!(
        best_bid_after.unwrap().amount,
        best_bid_before.unwrap().amount,
        "get_best_bid should remain accessible"
    );

    let ranked_bids_after = client.get_ranked_bids(&active_invoice_id);
    assert_eq!(
        ranked_bids_after.len(),
        ranked_bids_before.len(),
        "get_ranked_bids should remain accessible"
    );

    let investment_after = client.get_invoice_investment(&funded_invoice_id);
    assert_eq!(
        investment_after.amount, investment_before.amount,
        "get_invoice_investment should remain accessible"
    );

    let escrow_after = client.get_escrow_status(&funded_invoice_id);
    assert_eq!(
        escrow_after.amount, escrow_before.amount,
        "get_escrow_status should remain accessible"
    );
