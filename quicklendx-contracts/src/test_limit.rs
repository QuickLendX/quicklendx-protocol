...
fn test_limit_zero_returns_empty_all_endpoints() {
    ...
    assert_eq!(
        ...
        0,
        "Available invoices with limit=0 should return empty"
    );
    ...
    assert_eq!(
        ...
        0,
        "Investor investments with limit=0 should return empty"
    );

    let bid_history =
        client.get_bid_history_paged(&invoice_id, &Option::<BidStatus>::None, &0u32, &0u32);
    assert_eq!(
        bid_history.len(),
        0,
        "Bid history with limit=0 should return empty"
    );

    let investor_bids =
        client.get_investor_bids_paged(&investor, &Option::<BidStatus>::None, &0u32, &0u32);
    assert_eq!(
        investor_bids.len(),
        0,
        "Investor bids with limit=0 should return empty"
    );
    ...
    assert_eq!(
        currencies.len(),
        0,
        "Whitelisted currencies with limit=0 should return empty"
    );
}
...
fn test_overflow_protection() {
    ...
    assert_eq!(
        business_invoices.len(),
        0,
        "Overflow protection should return empty results"
    );
...
fn test_extreme_offset_validation() {
    ...
    assert_eq!(
        result.len(),
        0,
        "u32::MAX offset should return empty results"
    );
    ...
    assert_eq!(
        result2.len(),
        0,
        "Dangerous offset should return empty results"
    );
}
