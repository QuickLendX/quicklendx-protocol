...
mod test_investor_kyc {
    ...
    fn test_investor_moves_from_rejected_to_pending_on_resubmission() {
        ...
        assert!(
            !rejected.contains(&investor),
            "Should be removed from rejected list"
        );
    ...
    fn test_calculate_investment_limit_boundaries() {
        ...
        let low = client.calculate_investment_limit(
            &InvestorTier::Basic,
            &InvestorRiskLevel::Low,
            &100_000,
        );
        ...
        let medium = client.calculate_investment_limit(
            &InvestorTier::Silver,
            &InvestorRiskLevel::Medium,
            &100_000,
        );
        assert_eq!(medium, 150_000);

        let high = client.calculate_investment_limit(
            &InvestorTier::Gold,
            &InvestorRiskLevel::High,
            &100_000,
        );
        assert_eq!(high, 150_000);

        let very_high = client.calculate_investment_limit(
            &InvestorTier::VIP,
            &InvestorRiskLevel::VeryHigh,
            &100_000,
        );
        assert_eq!(very_high, 250_000);

        let zero_base =
            client.calculate_investment_limit(&InvestorTier::VIP, &InvestorRiskLevel::Low, &0);
        assert_eq!(zero_base, 0);
    }
