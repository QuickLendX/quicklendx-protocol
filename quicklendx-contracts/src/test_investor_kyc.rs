...
mod test_investor_kyc {
    ...
    fn create_verified_invoice(
        ...
        let currency = env.register_stellar_asset_contract_v2(business.clone()).address();
    ...
    fn mint_tokens(env: &Env, currency: &Address, to: &Address, amount: i128, spender: &Address) {
        ...
        token_client.approve(to, spender, &amount, &(env.ledger().sequence() + 1000));
    }
    ...
    fn test_is_investor_verified_state_transitions() {
        ...
        let _ = client.try_reject_investor(&investor, &String::from_str(&env, "Compliance issue"));
        let verification = client.get_investor_verification(&investor);
        assert_eq!(
            verification.unwrap().status,
            BusinessVerificationStatus::Rejected
    ...
    fn test_multiple_accepted_bids_create_multiple_investments() {
        ...
        // Mint tokens and approve for each invoice
        let inv1 = client.get_invoice(&invoice_id1);
        mint_tokens(&env, &inv1.currency, &investor, 10_000, &client.address);
        
        let inv2 = client.get_invoice(&invoice_id2);
        mint_tokens(&env, &inv2.currency, &investor, 15_000, &client.address);
        
        let inv3 = client.get_invoice(&invoice_id3);
        mint_tokens(&env, &inv3.currency, &investor, 12_000, &client.address);
...
// Analytics tests removed as methods are removed
}
